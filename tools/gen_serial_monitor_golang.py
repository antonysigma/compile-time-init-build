...
other_symbol = ~r"[^\n]*"i
log_symbol = uint32_type_alias ws ~r"catalog<" sc_message ws ">()"
sc_message = "sc::message<" sc_undefined ws closing_bracket
...
ascii_code = digits
undefined_string_id = delimiter ~"-?[0-9]+"i
...
class CIBLogging(NodeVisitor):
    ...
    def visit_log_symbol(self, node, visited_children) -> LogMessage:
        string_format, args = visited_children[3]
    ...
    def visit_sc_undefined(self, _, visited_children) -> tuple[bytes, ctype_array]:
        _, string_format_args, _, string_format, _ = visited_children
        return string_format, string_format_args

    def visit_sc_message(self, _, visited_children) -> bytes:
